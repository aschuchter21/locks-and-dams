...
public final class ModelPreviewChecks {
    static boolean started,ready,done,walk;
    static int tick;
    static ServerPlayer player;
    static ServerLevel level;
    static final BlockPos BASE=new BlockPos(2200,200,0);
    ...
    @SubscribeEvent public static void client(TickEvent.ClientTickEvent e) {
        ...
        if(walk&&mc.player!=null&&mc.player.getX()>BASE.getX()+5.3){walk=false;mc.options.keyUp.setDown(false);}
    ...
    @SubscribeEvent public static void server(TickEvent.ServerTickEvent e) {
        ...
        try {
            ...
            if(tick==225)shot="dev7-catwalk-walk.png";
            if(tick==300){
                walk=false;
                ...
                player.setGameMode(GameType.SPECTATOR);player.teleportTo(level,BASE.getX()+11,211,10,140,29);
                Minecraft.getInstance().execute(()->Minecraft.getInstance().options.hideGui=true);
            }
            if(tick==360)shot="dev7-lock-overview.png";
            if(tick==400)player.teleportTo(level,BASE.getX()-8,207,-7,-75,27);
            if(tick==460)shot="dev7-plumbing.png";
