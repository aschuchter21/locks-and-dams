...
public final class DeskChecks {
    ...
    @SubscribeEvent public static void tick(TickEvent.ServerTickEvent e){
        ...
        try{for(Fixture f:fixtures){var c=f.c;var controls=f.lock.deskControl();var desk=(ControlDeskEntity)level.getBlockEntity(f.desk);
            ...
            if(tick==10){check(f.c.fillChamber!=null&&f.c.drainChamber!=null,"Demo must use two-port inline circuits");check(controls.waterSelection==DeskControl.DRAIN&&controls.waterCommand()==0,"New demo must start low, DRAIN selected, idle");check(f.lock.gateLamp(0)==1&&f.lock.gateLamp(1)==1,"Initial gates not closed");for(int i=0;i<4;i++)check(Math.abs(c.hinge(level,i).getSpeed())<.01,"Clutch did not stop initial rotation: "+c.hinge(level,i).getSpeed());}
            ...
            if(tick==640){check(f.lock.gateLamp(0)==1,"Lower gate not closed for filling");f.water=f.lock.waterUnits();desk.operate(net.minecraftforge.common.util.FakePlayerFactory.getMinecraft(level),1,false,false);check(controls.waterSelection==DeskControl.FILL,"First selector click must request FILL");}
            if(tick>640&&tick<840)check(f.lock.waterUnits()==f.water,"Water moved during initial warning");
            if(tick==650)check(f.lock.status().contains("Fill warning:"),"Missing fill warning status: "+f.lock.status());
            ...
            if(tick==1801)check(f.lock.status().contains("At upper water level"),"Missing upper limit status: "+f.lock.status());
            ...
            if(tick==2820)check(f.lock.status().contains("At lower water level"),"Missing lower limit status: "+f.lock.status());
