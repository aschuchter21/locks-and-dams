    ...
    @SubscribeEvent public static void tick(TickEvent.ServerTickEvent e){
        ...
        try{for(Fixture f:fixtures){var c=f.c;var controls=f.lock.deskControl();var desk=(ControlDeskEntity)level.getBlockEntity(f.desk);
            ...
            if(tick>210&&tick<500){
                ...
                if(desk.output(false)>0){if(f.hornStart<0)f.hornStart=tick;f.hornLength++;check(f.firstOpen>=0&&tick-f.firstOpen>=80,"Horn sounded early");check(endSignal(f,false)==15&&endSignal(f,true)==0,"Horn did not reach its independent end contact");}
            }
            ...
            if(tick>=900&&tick<1100){check(f.lock.waterUnits()==f.water,"Water moved before restarted 10-second warning");check(f.lock.deskOutput(Direction.DOWN)==15&&desk.output(true)==15,"Warning output missing");check(endSignal(f,true)==15&&endSignal(f,false)==0,"Alarm did not reach its independent end contact");check(level.getSignal(f.lock.getBlockPos(),Direction.UP)==15,"Controller bottom alarm missing");}
